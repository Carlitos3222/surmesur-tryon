...
export default function SurmesurTryOn() {
  ...
  const s = {
    ...
    // Photo phase
    photoWrap: { maxWidth: '560px', margin: '0 auto', padding: '2rem 1.5rem' },
    hero: { textAlign: 'center', padding: '2.5rem 1.5rem 2rem', background: '#fff', borderBottom: '1px solid #e8e4df' },
    eyebrow: { fontSize: '0.65rem', letterSpacing: '0.22em', color: '#C9A96E', marginBottom: '0.8rem', fontFamily: 'sans-serif' },
    title: { fontSize: 'clamp(1.7rem,4vw,2.8rem)', fontWeight: 300, lineHeight: 1.2, marginBottom: '0.6rem' },
    gold: { color: '#C9A96E', fontStyle: 'italic' },
    sub: { fontSize: '0.8rem', color: '#999', fontFamily: 'sans-serif', marginBottom: '1.5rem' },
    stats: { display: 'flex', justifyContent: 'center', gap: '2.5rem', flexWrap: 'wrap' },
    statN: { fontSize: '1.3rem', fontWeight: 300, color: '#C9A96E' },
    statL: { fontSize: '0.6rem', letterSpacing: '0.12em', color: '#aaa', fontFamily: 'sans-serif' },
    ...
    uploadZone: { border: '1px dashed #ccc', borderRadius: '4px', padding: '2.5rem 1.5rem', textAlign: 'center', cursor: 'pointer', background: '#fafaf8', marginBottom: '0.75rem' },
    uploadIcon: { fontSize: '1.8rem', color: '#C9A96E', marginBottom: '0.5rem' },
    uploadTxt: { fontSize: '0.95rem', fontWeight: 300, color: '#333', marginBottom: '0.2rem' },
    uploadSub: { fontSize: '0.65rem', color: '#bbb', fontFamily: 'sans-serif' },
    btnRow: { display: 'flex', gap: '0.6rem', marginTop: '0.75rem' },
    btnBlack: { flex: 1, padding: '0.85rem', background: '#000', color: '#fff', border: 'none', cursor: 'pointer', fontSize: '0.7rem', letterSpacing: '0.1em', fontFamily: 'sans-serif' },
    btnOutline: { flex: 1, padding: '0.85rem', background: 'transparent', color: '#000', border: '1px solid #000', cursor: 'pointer', fontSize: '0.7rem', letterSpacing: '0.1em', fontFamily: 'sans-serif' },
    btnGhost: { flex: 1, padding: '0.85rem', background: 'transparent', color: '#888', border: '1px solid #ddd', cursor: 'pointer', fontSize: '0.7rem', letterSpacing: '0.1em', fontFamily: 'sans-serif' },
          ...
          <div style={s.mainCol}>
            ...
            {cameraActive && (
              <div style={{ marginBottom: '1.25rem' }}>
                <div style={s.camWrap}><video ref={videoRef} style={s.camVideo} autoPlay playsInline muted /></div>
                <div style={s.camHint}>⏱ 3 secondes après le bouton</div>
                <button onClick={capturePhoto} disabled={countdown !== null} style={s.capBtn}>
                  {countdown ? <span style={{ color: '#C9A96E', fontSize: '1.6rem' }}>{countdown}</span> : <div style={s.capInner} />}
                </button>
                {photoConfirmation && (
                  <>
                    <img src={photoConfirmation} alt="Preview" style={s.confirmImg} />
                    <div style={s.confirmBtns}>
                      <button style={s.btnBlack} onClick={confirmPhoto}>✓ UTILISER</button>
                      <button style={s.btnGhost} onClick={() => { setPhotoConfirmation(null); startCamera() }}>↺ REPRENDRE</button>
                    </div>
                  </>
                )}
